...
public class ListaAlberghiPerCittaProcedureDAO implements GenericProcedureDAO<String, List<Albergo>> {

    @Override
    public List<Albergo> execute(String input) throws DAOException, SQLException {

        List<Albergo> alberghi = new ArrayList<>();

        Connection conn = AppContext.getActiveConnection();

        CallableStatement cs = conn.prepareCall("{call ListaAlberghiPerCitta(?)}");
